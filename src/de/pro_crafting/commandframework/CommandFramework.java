...
public class CommandFramework implements CommandExecutor {
	...
	public boolean handleCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args) {
		for (int i = args.length; i >= 0; i--) {
			...
			if (commandMap.containsKey(cmdLabel)) {
				...
				de.pro_crafting.commandframework.Command command = method.getAnnotation(de.pro_crafting.commandframework.Command.class);
				...
				try {
					method.invoke(methodObject, new de.pro_crafting.commandframework.CommandArgs(sender, cmd, label, args,
		...
		defaultCommand(new de.pro_crafting.commandframework.CommandArgs(sender, cmd, label, args, 0));
		return true;
	}
	...
	public void registerCommands(Object obj) {
		for (Method m : obj.getClass().getMethods()) {
			if (m.getAnnotation(de.pro_crafting.commandframework.Command.class) != null) {
				de.pro_crafting.commandframework.Command command = m.getAnnotation(de.pro_crafting.commandframework.Command.class);
				if (m.getParameterTypes().length != 1 || m.getParameterTypes()[0] != de.pro_crafting.commandframework.CommandArgs.class) {
			...
			} else if (m.getAnnotation(de.pro_crafting.commandframework.Completer.class) != null) {
				de.pro_crafting.commandframework.Completer comp = m.getAnnotation(de.pro_crafting.commandframework.Completer.class);
				if (m.getParameterTypes().length > 1 || m.getParameterTypes().length == 0 || m.getParameterTypes()[0] != de.pro_crafting.commandframework.CommandArgs.class) {
	...
	public void registerCommand(Command command, String label, Method m, Object obj) {
		...
		if (map.getCommand(cmdLabel) == null) {
			org.bukkit.command.Command cmd = new de.pro_crafting.commandframework.BukkitCommand(cmdLabel, this, plugin);
	...
	public void registerCompleter(String label, Method m, Object obj) {
		String cmdLabel = label.replace(".", ",").split(",")[0].toLowerCase();
		if (map.getCommand(cmdLabel) == null) {
			org.bukkit.command.Command command = new de.pro_crafting.commandframework.BukkitCommand(cmdLabel, this, plugin);
			map.register(plugin.getName(), command);
		}
		if (map.getCommand(cmdLabel) instanceof de.pro_crafting.commandframework.BukkitCommand) {
			de.pro_crafting.commandframework.BukkitCommand command = (BukkitCommand) map.getCommand(cmdLabel);
			if (command.completer == null) {
				command.completer = new de.pro_crafting.commandframework.BukkitCompleter();
			}
			...
			try {
				...
				if (field.get(command) == null) {
					de.pro_crafting.commandframework.BukkitCompleter completer = new de.pro_crafting.commandframework.BukkitCompleter();
					completer.addCompleter(label, m, obj);
					field.set(command, completer);
				} else if (field.get(command) instanceof de.pro_crafting.commandframework.BukkitCompleter) {
					de.pro_crafting.commandframework.BukkitCompleter completer = (BukkitCompleter) field.get(command);
					completer.addCompleter(label, m, obj);
				} else {
					plugin.getLogger().warning("Unable to register tab completer " + m.getName()
